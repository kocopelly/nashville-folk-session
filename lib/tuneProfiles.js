...
export function computeTuneProfiles(sessions, tunes) {
  ...
  for (const session of sorted) {
    for (const set of session.sets) {
      const setTuneIds = set.tunes.map(e => normalizeTuneEntry(e).tuneId);
      ...
      for (const entry of set.tunes) {
        ...
        const partners = set.tunes
          .map(e => normalizeTuneEntry(e))
          .filter(e => e.tuneId !== tuneId)
          .map(e => ({ id: e.tuneId, name: tunes[e.tuneId]?.name ?? e.tuneId }));
