...
export function computeKeyJourneys(sorted) {
  return sorted.map((session) => {
    ...
    return { id: session.id, date: session.date, keys };
  });
}
...
export function buildTimeSeries(sorted, tunes, typeBreakdown, keyDistribution, repertoireGrowth) {
  ...
  // Cumulative unique tunes split into Jig / Reel / Other bands (stacked-area repertoire growth).
  // Each band = running count of DISTINCT tunes of that group first seen up to & including each session.
  /** @type {{ Reel: Set<string>, Jig: Set<string>, Other: Set<string> }} */
  const seenByBand = { Reel: new Set(), Jig: new Set(), Other: new Set() };
  /** @type {{ Reel: number[], Jig: number[], Other: number[] }} */
  const cumulativeByType = { Reel: [], Jig: [], Other: [] };
  const bandOf = (/** @type {string} */ type) => (type === 'reel' ? 'Reel' : type === 'jig' ? 'Jig' : 'Other');
  for (const session of sorted) {
    for (const set of session.sets) {
      for (const entry of set.tunes) {
        const { tuneId } = normalizeTuneEntry(entry);
        const band = bandOf(tunes[tuneId]?.type ?? 'unknown');
        seenByBand[band].add(tuneId);
      }
    }
    cumulativeByType.Reel.push(seenByBand.Reel.size);
    cumulativeByType.Jig.push(seenByBand.Jig.size);
    cumulativeByType.Other.push(seenByBand.Other.size);
  }
