...
export function computeSessionDNA(sorted, tunes) {
  return sorted.map((session) => {
    const tuneCount = session.sets.reduce((n, s) => n + s.tunes.length, 0);
    const sets = session.sets.map((set) => {
      ...
      const dominantType =
        set.label && set.label !== 'mixed'
          ? set.label.replace(/s$/, '')
          : Object.entries(setTypeCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'unknown';

      return {
        label: set.label || dominantType,
        type: dominantType,
        colorClass: TYPE_COLORS[dominantType] || TYPE_COLORS.mixed,
...
export function computeKeyJourneys(sorted) {
  return sorted.map((session) => {
    const keys = [];
    for (const set of session.sets) {
      const setKeys = set.tunes.map((e) => normalizeTuneEntry(e).key).filter(Boolean);
...
export function buildTimeSeries(sorted, tunes, typeBreakdown, keyDistribution, repertoireGrowth) {
  ...
  return {
    ...
    tunesPerSession: sorted.map((s) => s.sets.reduce((n, set) => n + set.tunes.length, 0)),
