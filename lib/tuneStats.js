...
export function computeVelocity(tunePlays, sorted) {
  ...
  for (const t of tunePlays) {
    // Recent activity: how many of the last 3 sessions featured this tune.
    const recentCount = t.sessionIds.filter((/** @type {string} */ id) => recentIds.includes(id)).length;

    // "Heating up" = played in 2+ of the last 3 sessions — what's hot right now,
    // independent of whether it's also a long-run standard.
    if (recentCount >= 2) {
      heating.push({ id: t.id, name: t.name, count: t.count, sessions: t.sessionCount, recent: recentCount });
    }

    // Standards = long-run staples (3+ sessions total).
  ...
  // Rank heating by recent presence, then total plays.
  heating.sort((a, b) => b.recent - a.recent || b.count - a.count);
