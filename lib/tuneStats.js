...
export function computeVelocity(tunePlays, sorted) {
  ...
  for (const t of tunePlays) {
    ...
    // it's been gone, so a well-loved tune gone a few sessions can outrank an
    // obscure one gone forever.
    //   dustScore = log2(1+plays) * log2(1+sessionsSince)
    ...
    // (absent for at least half the recency window) so current tunes drop out.
    if (t.sessionCount >= 2) {
      ...
      const dustFloor = Math.max(2, Math.ceil(windowSize / 2));
      if (sessionsSince >= dustFloor) {
        const dustScore = Math.log2(1 + t.count) * Math.log2(1 + sessionsSince);
        dormant.push({
          id: t.id,
          name: t.name,
          count: t.count,
  ...
  dormant.sort((a, b) => b.dustScore - a.dustScore || b.count - a.count || b.sessionsSince - a.sessionsSince);
