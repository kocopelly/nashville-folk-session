...
export function computeVelocity(tunePlays, sorted) {
  ...
  for (const t of tunePlays) {
    const recentCount = t.sessionIds.filter((id) => recentIds.includes(id)).length;
 ...
 * @param {any[]} sorted
 * @param {Record<string, any>} tunes
 * @returns {{ pairCounts: Record<string, number>, pairings: any[] }}
 */
export function computePairings(sorted, tunes) {
  ...
  const pairings = Object.entries(pairCounts)
    ...
    .map(([pair, count]) => {
      ...
      return { tuneA: tunes[a]?.name ?? a, tuneB: tunes[b]?.name ?? b, count };
    })
    .sort((a, b) => b.count - a.count)
    .slice(0, 15);
