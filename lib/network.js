...
export function computeNetwork(sorted, tunes, pairCounts) {
  ...
  /** @type {{ id: string; name: string; type: string; degree: number; totalWeight: number; clustering: number; betweenness: number }[]} */
  ...
  for (const s of networkNodeIds) {
    ...
    while (queue.length > 0) {
      const v = /** @type {string} */ (queue.shift());
      stack.push(v);
  ...
  for (const id of networkNodeIds) {
    if (!visited.has(id)) {
      ...
      const q = [id];
      while (q.length > 0) {
        const curr = /** @type {string} */ (q.shift());
